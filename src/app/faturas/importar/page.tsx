...
export default function ImportarFaturasPage() {
  ...
  const handleImportarFaturas = async () => {
    ...
    try {
      ...
      // Opcional: Limpar faturasPreview se a importação for bem-sucedida
      // setFaturasPreview([])
    } catch (error) {
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto py-6 sm:px-6 lg:px-8">
        <div className="px-4 py-6 sm:px-0">
          <h1 className="text-3xl font-bold text-gray-900 mb-6">Importar Faturas</h1>
                  ...
                  <TableBody>
                    {importacaoResultado.detalhes.map((detalhe, index) => (
                      <TableRow key={index} className={detalhe.erro ? 'bg-red-50' : 'bg-green-50'}>
                        ...
                        <TableCell>
                          {detalhe.erro || 
                           `Fatura ${detalhe.fatura?.numero_fatura} do cliente ${detalhe.fatura?.cliente_id} (R$ ${formatarMoeda(detalhe.fatura?.valor_centavos || 0)}) importada.`}
                        </TableCell>
                      </TableRow>
                    ))}
                  </TableBody>
