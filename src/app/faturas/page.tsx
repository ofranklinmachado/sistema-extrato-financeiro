...
export default function FaturasImportadasPage() {
  ...
  return (
    <div className="space-y-8">
      <h1 className="text-3xl font-bold text-gray-900">Faturas Importadas (Staging)</h1>
              ...
              <TableBody>
                {processResult.detalhes.map((detalhe, index) => (
                  <TableRow key={index} className={detalhe.erro ? 'bg-red-50' : 'bg-green-50'}>
                    ...
                    <TableCell>
                      {detalhe.erro || 
                       `Fatura do cliente ${detalhe.cliente} (${formatarMoeda(detalhe.valor)}) com prazo ${detalhe.dataPagamento}.`}
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
